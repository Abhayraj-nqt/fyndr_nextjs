...
>(({ className, ...props }, ref) => (
  <ComboboxPrimitive.Anchor
    ...
    ref={ref}
    className={cn(
      "relative flex h-9 w-full items-center justify-between gap-2 rounded-md border border-slate-200 bg-transparent px-3 py-2 shadow-xs data-focused:ring-1 data-focused:ring-slate-950 dark:border-slate-800 dark:data-focused:ring-slate-300",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ComboboxPrimitive.Input
    ...
    ref={ref}
    className={cn(
      "flex h-9 w-full rounded-md bg-transparent text-base placeholder:text-slate-500 focus:outline-hidden disabled:cursor-not-allowed disabled:opacity-50 md:text-sm dark:placeholder:text-slate-400",
      className,
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <ComboboxPrimitive.Trigger
    ...
    ref={ref}
    className={cn(
      "flex shrink-0 items-center justify-center rounded-r-md border-slate-200 bg-transparent text-slate-500 transition-colors hover:text-slate-950/80 focus-visible:outline-hidden disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-800 dark:text-slate-400 dark:hover:text-slate-50/80",
      className,
    )}
    {...props}
  >
...
>(({ className, ...props }, ref) => (
  <ComboboxPrimitive.Cancel
    ...
    ref={ref}
    className={cn(
      "-translate-y-1/2 absolute top-1/2 right-1 flex h-6 w-6 items-center justify-center rounded-sm bg-white opacity-70 ring-offset-white transition-opacity hover:opacity-100 focus:outline-hidden focus:ring-2 focus:ring-slate-950 focus:ring-offset-2 disabled:pointer-events-none dark:bg-slate-950 dark:ring-offset-slate-950 dark:focus:ring-slate-300",
      className,
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <ComboboxPrimitive.BadgeItem
    ...
    ref={ref}
    className={cn(
      "inline-flex items-center justify-between gap-1 rounded-sm bg-slate-100 px-2 py-0.5 dark:bg-slate-800",
      className,
    )}
    {...props}
  >
    <span className="truncate text-[13px] text-slate-900 dark:text-slate-50">
      {children}
    </span>
    <ComboboxPrimitive.BadgeItemDelete
      data-slot="combobox-badge-item-delete"
      className="shrink-0 rounded p-0.5 opacity-70 ring-offset-white transition-opacity hover:opacity-100 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-slate-950 data-highlighted:bg-red-500 dark:ring-offset-slate-950 dark:focus-visible:ring-slate-300 dark:data-highlighted:bg-red-900"
    >
      <X className="h-3 w-3" />
    </ComboboxPrimitive.BadgeItemDelete>
  </ComboboxPrimitive.BadgeItem>
...
>(({ sideOffset = 6, className, children, ...props }, ref) => (
  <ComboboxPrimitive.Portal>
    <ComboboxPrimitive.Content
      ...
      className={cn(
        "data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 relative z-50 max-h-fit min-w-[var(--dice-anchor-width)] origin-[var(--dice-transform-origin)] overflow-hidden rounded-md border border-slate-200 bg-white p-1 text-slate-950 shadow-md data-[state=closed]:animate-out data-[state=open]:animate-in dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50",
        className,
      )}
      {...props}
    >
...
>(({ className, ...props }, ref) => (
  <ComboboxPrimitive.GroupLabel
    ...
    ref={ref}
    className={cn(
      "px-2 py-1.5 font-semibold text-slate-500 text-xs dark:text-slate-400",
      className,
    )}
    {...props}
  />
...
>(({ className, children, outset, ...props }, ref) => (
  <ComboboxPrimitive.Item
    ...
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 text-sm outline-hidden data-disabled:pointer-events-none data-highlighted:bg-slate-100 data-highlighted:text-slate-900 data-disabled:opacity-50 dark:data-highlighted:bg-slate-800 dark:data-highlighted:text-slate-50",
