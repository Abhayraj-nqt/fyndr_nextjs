...
const QtySelector3 = ({
  ...
  // TODO: Need to review: Initialize quantity from cart
  useEffect(() => {
  ...
  // TODO: Need to review: Auto-select location if only one available
  useEffect(() => {
  ...
  const finalizeQuantityUpdate = async (newQty: number) => {
  ...
  const handleQtyChange = async (newQty: number) => {
    ...
    try {
        ...
        const pendingAction = () => {
          startTransition(async () => {
            try {
              ...
              // Check if appointment booking is required
              if (
                selectedLocation &&
                requiresAppointmentBooking(offer, selectedLocation)
              ) {
                // Open appointment modal
                const appointmentPendingAction = () => {
                  finalizeQuantityUpdate(qty);
                };
              ...
              console.error("Error in location pending action:", error);
      ...
      // Location is already selected or auto-selected
      const selectedLocation = getCampaignLocation(campaignId);
        ...
        const appointmentPendingAction = () => {
          startTransition(async () => {
            try {
              ...
              console.error("Error in appointment pending action:", error);
              setError("Failed to update quantity");
            } finally {
              setLoading(false);
            }
      ...
      // No special requirements - process normally
      startTransition(async () => {
        try {
          await finalizeQuantityUpdate(qty);
        } catch (error) {
